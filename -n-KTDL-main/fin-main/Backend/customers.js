...
router.post('/add', async (req, res) => {
    try {
        const { FirstName, LastName, Company, City, Country, Phone1, Phone2, Email, SubscriptionDate, Password, Sex, Age} = req.body;

        // Kiểm tra nếu các trường bắt buộc không có
        if (!FirstName || !LastName || !City || !Phone1 || !Email || !Password) {
...
router.patch('/update/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { CustID, FirstName, LastName, company, city, country, phone1, phone2, Email, SubscriptionDate, Password, Sex, Age} = req.body;
