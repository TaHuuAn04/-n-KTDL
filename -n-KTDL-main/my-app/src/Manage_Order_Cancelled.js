...
function Manage_Order_Cancelled () {
    ...
    useEffect(() => {
        const fetchDeliveredOrders = async () => {
            try {
                const response = await axios.get('http://localhost:3001/getOrder_Delivering', {
            ...
            if (data.success) {
                ...
                setOrders(prevOrders => prevOrders.map(orderGroup => ({
                    ...
                    orders: orderGroup.orders.map(order =>
                        order.SKU === SKU ? { ...order, Status: 'Cancelled' } : order
                    )
                })));
                alert('Sản phẩm trong đơn hàng đã được loại khỏi đơn hàng đang giao!');
            } else {
                alert('Sản phẩm trong đơn hàng đã được loại khỏi đơn hàng đang giao!');
            }
        } catch (error) {
    ...
    return (
        ...
        <div className="management-main">
            <div className="backgr-management">
                <div className="title-of">CANCEL ORDER</div>
                <div className="fol-details">
                    <p>Nếu quý khách muốn hủy bỏ sản phẩm nào trong đơn hàng thì hãy Click vào nút <b>Cancelled</b></p>
                </div>
                ...
                <div key={orderGroup._id}>
                    ...
                    {orderGroup.orders.map(order => (
                        <div key={order.SKU} className="product-detail-management-main" >
                            ...
                            <button onClick={() => handleReturn(order.SKU)}>Cancelled</button>
                            </div>
                            </div>
...
export default Manage_Order_Cancelled;